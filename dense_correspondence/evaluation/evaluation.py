...
class DenseCorrespondenceEvaluation(object):
    ...
    def evaluate_single_network_cross_instance(self, network_name, full_path_cross_instance_labels, save=False):
        ...
        dcn.eval()
        dataset = dcn.load_training_dataset()
        return DenseCorrespondenceEvaluation.evaluate_network_cross_instance(dcn, dataset,
    ...
    def evaluate_network_cross_scene_keypoints(dcn, dataset, full_path_cross_instance_labels):
        ...
        for subset in itertools.combinations(cross_instance_keypoint_labels, 2):
            ...
            dataframe_list_temp = \
                DenseCorrespondenceEvaluation.single_image_pair_cross_scene_keypoints_quantitative_analysis(dcn,
                                                                                                            dataset,
                                                                                                            keypoint_data_a,
                                                                                                            keypoint_data_b,
                                                                                                            res_a,
    ...
    def plot_descriptor_colormaps(res_a, res_b, descriptor_image_stats=None,
                                  mask_a=None, mask_b=None, plot_masked=False, descriptor_norm_type="mask_image", output_is_normalized=True):
        """
        ...
        if output_is_normalized:
            # you may wonder why to perfrm `dc_plotting.normalize_descriptor` when `output_is_normalized=True`
            # because "dc_plotting.normalize_descriptor" encodes values [a ,b] -> [0, 1] range for plotting
            # `output_is_normalized=True` means that descriptor is normalized in L2 sense so values are in [-1, 1] range
            res_a_norm = dc_plotting.normalize_descriptor(res_a, {'min': -1.0, 'max': 1.0})
            res_b_norm = dc_plotting.normalize_descriptor(res_b, {'min': -1.0, 'max': 1.0})
        elif descriptor_image_stats is None:
    ...
    def single_cross_scene_image_pair_quantitative_analysis(dcn, dataset, scene_name_a,
        ...
        for i in range(len(img_a_pixels)):
            ...
            pd_template = DenseCorrespondenceEvaluation.compute_descriptor_match_statistics(depth_a,
                                                                                            depth_b, mask_a, mask_b,
                                                                                            uv_a, uv_b, pose_a, pose_b,
                                                                                            res_a,
                                                                                            res_b,
        ...
        for i in range(len(img_a_pixels)):
            ...
            for j in range(J):
                ...
                if different_view_a_idx is None:
                    ...
                    continue
                diff_rgb_a, diff_depth_a, diff_mask_a, diff_pose_a = dataset.get_rgbd_mask_pose(scene_name_a,
                                                                                                different_view_a_idx)
                diff_depth_a = np.asarray(diff_depth_a)
                diff_mask_a = np.asarray(diff_mask_a)
                (uv_a_vec, diff_uv_a_vec) = correspondence_finder.batch_find_pixel_correspondences(depth_a, pose_a,
                                                                                                   diff_depth_a,
                                                                                                   diff_pose_a,
                                                                                                   uv_a=uv_a)
                if uv_a_vec is None:
            ...
            for k in range(K):
                ...
                if different_view_b_idx is None:
                    logging.info("no frame with sufficiently different pose found, continuing")
                    continue
                diff_rgb_b, diff_depth_b, diff_mask_b, diff_pose_b = dataset.get_rgbd_mask_pose(scene_name_b,
                                                                                                different_view_b_idx)
                diff_depth_b = np.asarray(diff_depth_b)
                diff_mask_b = np.asarray(diff_mask_b)
                (uv_b_vec, diff_uv_b_vec) = correspondence_finder.batch_find_pixel_correspondences(depth_b, pose_b,
                                                                                                   diff_depth_b,
                                                                                                   diff_pose_b,
                                                                                                   uv_a=uv_b)
                if uv_b_vec is None:
    ...
    def compute_3d_position(uv, depth, camera_intrinsics_matrix, camera_to_world):
        ...
        """
        pos_in_camera_frame = correspondence_finder.pinhole_projection_image_to_world(uv, depth,
                                                                                      camera_intrinsics_matrix)
    ...
    def single_cross_scene_image_pair_qualitative_analysis(dcn, dataset, scene_name_a,
        ...
        DenseCorrespondenceEvaluation.single_image_pair_qualitative_analysis(dcn, dataset, rgb_a, rgb_b, mask_a, mask_b,
                                                                             num_matches)
        return rgb_a, rgb_b
    ...
    def single_image_pair_qualitative_analysis(dcn, dataset, rgb_a, rgb_b, mask_a, mask_b, num_matches, output_is_normalized=True):
        ...
        gray_a_numpy = cv2.cvtColor(np.asarray(rgb_a), cv2.COLOR_BGR2GRAY)
        gray_b_numpy = cv2.cvtColor(np.asarray(rgb_b), cv2.COLOR_BGR2GRAY)
        correspondence_image_pair = cv2.drawMatches(gray_a_numpy, kp1, gray_b_numpy, kp2, matches, flags=2,
                                                    outImg=gray_b_numpy)
    ...
    def single_image_pair_cross_scene_keypoints_quantitative_analysis(dcn, dataset, keypoint_data_a,
        ...
        for kp_name, data_a in keypoint_data_a['keypoints'].iteritems():
            ...
            for order in ordering:
                ...
                uv_1 = DCE.clip_pixel_to_image_size_and_round((data[idx_1]['u'], data[idx_2]['v']), image_width,
                                                              image_height)
    ...
    def parse_cross_scene_data(dataset):
        ...
        for path in evaluation_labeled_data_paths:
            cross_scene_data_full_path = utils.convert_data_relative_path_to_absolute_path(path,
                                                                                           assert_path_exists=True)
    ...
    @staticmethod
    def evaluate_network_qualitative(dcn, dataset, num_image_pairs=5, randomize=False, scene_type=None, output_is_normalized=True):
        results = DenseCorrespondenceEvaluation.evaluate_network_qualitative_without_plotting(dcn, dataset,
                                                                                              num_image_pairs,
                                                                                              randomize, scene_type, output_is_normalized=output_is_normalized)

        print "\n\n-----------Train Data Evaluation----------------"
        train_evals = results['train_evals']
        for i, (combined_image, comment) in enumerate(train_evals):
            print comment
            DenseCorrespondenceEvaluation.plot_image(combined_image, 15, 15)

        print "\n\n-----------Test Data Evaluation----------------"
        test_evals = results['test_evals']
        for combined_image, comment in test_evals:
            print comment
            DenseCorrespondenceEvaluation.plot_image(combined_image, 15, 15)

        if 'caterpillar_test_results' in results.keys():
            print "\n\n-----------Test Data Evaluation----------------"
            caterpillar_test_evals = results['caterpillar_test_evals']
            for combined_image, comment in caterpillar_test_evals:
                print comment
                DenseCorrespondenceEvaluation.plot_image(combined_image, 15, 15)

    @staticmethod
    def plot_image(img, hieght=10, width=15):
        fig, axes = plt.subplots(nrows=1, ncols=1)
        fig.set_figheight(hieght)
    ...
    def evaluate_network_qualitative_without_plotting(dcn, dataset, num_image_pairs=5, randomize=False,
        ...
        train_results = []
        for scene_name, img_pair in zip(scene_names, img_pairs)[:num_image_pairs]:
            comment = "Image pair ({}, {})".format(img_pair[0], img_pair[1])
            res = DenseCorrespondenceEvaluation.single_same_scene_image_pair_qualitative_analysis(dcn,
                                                                                                  dataset,
                                                                                                  scene_name,
                                                                                                  img_pair[0],
                                                                                                  img_pair[1],
        ...
        result = {'train_evals': train_evals, 'test_evals': test_evals}
        if caterpillar_test_results:
            result.update({'caterpillar_test_evals': caterpillar_test_evals})
    ...
    def combine_qualitative_evaluation(evaluation):
        """
        evaluation is a dictionary of:
            2 images with correspondences between each other
            2 desciprtors
    ...
    def run_cross_instance_keypoint_evaluation_on_network(model_folder, path_to_cross_instance_labels,
        ...
        path_to_cross_instance_labels = utils.convert_data_relative_path_to_absolute_path(path_to_cross_instance_labels,
                                                                                          assert_path_exists=True)
    ...
    def make_2d_cluster_plot(dcn, dataset, plot_background=False):
        ...
        for i in range(100):
            ...
            object_uv_samples = correspondence_finder.random_sample_from_masked_image_torch(mask_torch,
                                                                                            num_samples_per_image)
            background_uv_samples = correspondence_finder.random_sample_from_masked_image_torch(mask_inv,
                                                                                                num_samples_per_image / num_objects)
        ...
        if not use_3d:
            ...
            if plot_background:
                plt.scatter(descriptors_background_samples[:, 0], descriptors_background_samples[:, 1], alpha=0.5,
                            label="background")
            plt.legend()
            plt.show()
        ...
        if use_3d:
            for key, value in descriptors_known_objects_samples_xy.iteritems():
                plt.scatter(value[:, 0], value[:, 1], alpha=0.5, label=key)
            if plot_background:
                plt.scatter(descriptors_background_samples_xy[:, 0], descriptors_background_samples_xy[:, 1], alpha=0.5,
                            label="background")
            plt.legend()
            plt.show()
            ...
            for key, value in descriptors_known_objects_samples_yz.iteritems():
                plt.scatter(value[:, 0], value[:, 1], alpha=0.5, label=key)
            if plot_background:
                plt.scatter(descriptors_background_samples_yz[:, 0], descriptors_background_samples_yz[:, 1], alpha=0.5,
                            label="background")
            plt.legend()
            plt.show()
            ...
            for key, value in descriptors_known_objects_samples_xz.iteritems():
                plt.scatter(value[:, 0], value[:, 1], alpha=0.5, label=key)
            if plot_background:
                plt.scatter(descriptors_background_samples_xz[:, 0], descriptors_background_samples_xz[:, 1], alpha=0.5,
                            label="background")
            plt.legend()
            plt.show()
...
class DenseCorrespondenceEvaluationPlotter(object):
    ...
    @staticmethod
    def log_cdf(logger, metric_name, data, num_bins, x_axis_scale_factor=1):
        DCEP = DenseCorrespondenceEvaluationPlotter
        xs, ys = DCEP.compute_cdf(data, num_bins, x_axis_scale_factor)
        for x, y in zip(xs, ys):
            logger.log(metric_name, x=x, y=y)
        logger.send_logs()
    ...
    @staticmethod
    def log_pixel_match_error(logger, df, title_prefix, num_bins=100, masked=False, iteration=0):
        DCEP = DenseCorrespondenceEvaluationPlotter
        ...
        # rescales the pixel distance to be relative to the diagonal of the image
        x_axis_scale_factor = 800
        plot_title = title_prefix + '_' + metric_name + '_' + str(iteration)
        DCEP.log_cdf(logger, plot_title, data, num_bins=num_bins, x_axis_scale_factor=x_axis_scale_factor)

        auc_title = title_prefix + '_' + metric_name
        DCEP.log_aac(logger, df, metric_name, auc_title, iteration)

    @staticmethod
    def log_across_object_best_match(logger, df, title_prefix, num_bins=100, iteration=0):
        DCEP = DenseCorrespondenceEvaluationPlotter
        plot_title = title_prefix + '_descriptor_best_match_l2' + '_' + str(iteration)
        metric_name = 'norm_diff_descriptor_best_match'
        data = df[metric_name]
        DCEP.log_cdf(logger, plot_title, data, num_bins=num_bins)

        title = title_prefix + '_descriptor_best_match_l2'
        DCEP.log_aac(logger, df, metric_name, title, iteration)
    ...
    @staticmethod
    def log_descriptor_accuracy(logger, df, title_prefix, num_bins=100, masked=False, iteration=0):
        DCEP = DenseCorrespondenceEvaluationPlotter
        ...
        title = title_prefix + '_3D_match_error_L2(cm)' + ('_masked' if masked else '')
        DCEP.log_aac(logger, df, metric_name, title, iteration)
    ...
    @staticmethod
    def log_norm_diff_ground_truth(logger, df, title_prefix, num_bins=100, masked=False, iteration=0):
        DCEP = DenseCorrespondenceEvaluationPlotter

        metric_name = 'norm_diff_descriptor_ground_truth'
        data = df[metric_name]

        plot_title = title_prefix + '_descriptor_match_error_l2_' + str(iteration)
        DCEP.log_cdf(logger, plot_title, data, num_bins=num_bins)

        title = title_prefix + '_descriptor_match_error_l2'
        DCEP.log_aac(logger, df, metric_name, title, iteration)
    ...
    @staticmethod
    def log_fraction_false_positives(logger, df, title_prefix, num_bins=100, masked=False, iteration=0):
        DCEP = DenseCorrespondenceEvaluationPlotter
        ...
        title = title_prefix + '_fraction_false_positives' + ('_masked' if masked else '')
        DCEP.log_aac(logger, df, metric_name, title, iteration)
    ...
    @staticmethod
    def log_average_l2_false_positives(logger, df, title_prefix, num_bins=100, masked=False, iteration=0):
        DCEP = DenseCorrespondenceEvaluationPlotter
        ...
        title = title_prefix + '_average_l2_distance_for_false_positives' + ('_masked' if masked else '')
        DCEP.log_aac(logger, df, metric_name, title, iteration)
    ...
    @staticmethod
    def log_metrics_from_dataframe(logger, path_to_df_csv, title_prefix='', iteration=0):
        """
        Same as DCEP.run_on_single_dataframe() but send logs to logger instead of plotting
        """
        DCEP = DenseCorrespondenceEvaluationPlotter
        path_to_csv = utils.convert_data_relative_path_to_absolute_path(path_to_df_csv, assert_path_exists=True)
        df = pd.read_csv(path_to_csv, index_col=0, parse_dates=True)
        use_masked_plots = 'is_valid_masked' in df

        # pixel match error
        DCEP.log_pixel_match_error(logger, df, title_prefix=title_prefix, iteration=iteration)
        if use_masked_plots:
            DCEP.log_pixel_match_error(logger, df, title_prefix=title_prefix, masked=True, iteration=iteration)

        # 3D match error
        DCEP.log_descriptor_accuracy(logger, df, title_prefix=title_prefix, iteration=iteration)
        if use_masked_plots:
            DCEP.log_descriptor_accuracy(logger, df, title_prefix=title_prefix, masked=True, iteration=iteration)

        # norm difference of the ground truth match (should be 0)
        DCEP.log_norm_diff_ground_truth(logger, df, title_prefix=title_prefix, iteration=iteration)

        # fraction false positives
        DCEP.log_fraction_false_positives(logger, df, title_prefix=title_prefix, iteration=iteration)
        if use_masked_plots:
            DCEP.log_fraction_false_positives(logger, df, title_prefix=title_prefix, masked=True, iteration=iteration)

        # average l2 false positives
        DCEP.log_average_l2_false_positives(logger, df, title_prefix=title_prefix, iteration=iteration)
        if use_masked_plots:
            DCEP.log_average_l2_false_positives(logger, df, title_prefix=title_prefix, masked=True, iteration=iteration)
